...
try:
    ...
    # Si da error trato de buscarlo en otra web
        print("El link original no funciona se busca en otro lado")
        url = 'http://www.ambito.com/economia/mercados/riesgo-pais/'
        with urlopen(url) as html_txt:
            soup = BeautifulSoup(html_txt, 'lxml')

        element = soup.select("#puntos")
        risk_today = int(element[0].select("span > strong")[0].text)
        print(risk_today)
